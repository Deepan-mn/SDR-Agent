...
class GmailToolsClass:
    ...
    def fetch_recent_emails(self, max_results=50):
        try:
            ...
            query = f'after:{one_day_ago} to:me from:customer.johnnydepp@gmail.com'
    ...
    def _get_email_info(self, user_id, msg_id):
        ...
        from_header = next((header['value'] for header in headers if header['name'].lower() == 'from'), 'Unknown')
        sender = re.search(r'<(.*?)>', from_header)
        sender = sender.group(1) if sender else from_header.split()[-1]
